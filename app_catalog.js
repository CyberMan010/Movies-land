...
function filterMoviesByCatalog2(category) {
  ...
  const filteredMovies = allMovies.filter((movie) => {
    ...
    return (
      movie.catalog2 && movie.catalog2.toLowerCase() === category.toLowerCase()
    );
  });
...
function filterMoviesByCatalog(category) {
  ...
  const filteredMovies = allMovies.filter((movie) => {
    ...
    return (
      movie.catalog && movie.catalog.toLowerCase() === category.toLowerCase()
    );
  });
...
document
  .getElementById("top-3")
  .addEventListener("click", () => filterMoviesByCatalog2("3"));
document
  .getElementById("most-viewed")
  .addEventListener("click", () => filterMoviesByCatalog2("1"));
document
  .getElementById("sports")
  .addEventListener("click", () => filterMoviesByCatalog("sports"));
document
  .getElementById("historical")
  .addEventListener("click", () => filterMoviesByCatalog("history"));
document
  .getElementById("business")
  .addEventListener("click", () => filterMoviesByCatalog("business"));
...
function createCard(data, key) {
  ...
  details.innerHTML = `
    <h2>${data.title} <br><span>${data.personname}</span></h2>
    <div class="tags">
      <a href="details.html" class="buttonDetails" onclick='storeData(${JSON.stringify(
        data
      )})'>MORE DETAILS</a>
    </div>
    <div class="info">
